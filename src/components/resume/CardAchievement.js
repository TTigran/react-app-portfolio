...
const CardAchievement = ({title, subTitle, result, des, achievementImages}) => {
    const [achImages, setAchImages] = useState(achievementImages)
    return (
        <div className="w-full h-1/3 group flex" style={{height:"705px"}}>
            <div className="w-10 h-[6px] bgOpacity mt-16 relative">
        <span
            className="absolute w-5 h-5 rounded-full -top-2 -left-3 flex justify-center items-center bg-black bg-opacity-60">
          <span
              className="w-3 h-3 rounded-full bg-bodyColor inline-flex group-hover:bg-designColor duration-300"></span>
        </span>
            </div>
            <div
                ...
                {achievementImages &&
                    <div className="mt-4">
                        <div className={achievementImages.length >0 ? "grid grid-cols-3 gap-4" : ""}>
